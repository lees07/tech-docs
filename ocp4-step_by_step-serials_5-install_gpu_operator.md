# OpenShift 4 离线环境搭建 step-by-step:5 - 安装 gpu operator

## 安装 nfd operator
在控制台的 OperatorHub 页面查询 nfd 点击找到的 nfd operator 进行安装  
![查找 nfd operator](./images/01-seek-nfd-operator.png)  

点击 Install 按钮安装 nfd operator  
![安装 nfd operator](./images/02-install-nfd-operator.png)  

使用默认的配置安装 nfd operator, 点击 Install 按钮继续  
![使用默认配置安装](./images/03-install-nfd-operator-continue.png)  

若安装 pending, 可能需要清理环境, 重新安装来解决  
![nfd operator 安装 pending ](./images/04-nfd-operator-install-pending.png)
...
oc get sub,csv -n openshift-operator
...
安装成功  
![nfd operator 安装成功](./images/05-nfd-operator-install-success.png)  

进入 nfd operator 的 node feature discovery 标签页, 点击按钮创建 node feature discovery  
![创建 node feature discovery](./images/06-create-node-feature-discovery.png)  

创建NFD成功  
![创建NFD成功](./images/07-node-feature-discovery-ready.png)  
...
![查找 nvidia gpu operator](./images/08-seek-gpu-operator.png)  

点击 Install 按钮安装 nvidia gpu operator  
![安装 nvidia gpu operator](./images/09-install-nvidia-gpu-operator.png)  

使用默认的配置安装 nvidia gpu operator, 点击 Install 按钮继续  
![使用默认配置安装](./images/10-install-nvidia-gpu-operator-continue.png)  

安装成功  
![nvidia gpu operator 安装成功](./images/12-nvidia-gpu-operator-intsall-success.png)  

进入 nvidia gpu operator 的 clusterpolicy 标签页, 点击按钮创建 clusterpolicy  
![创建 clusterpolicy](./images/13-create-cluster-policy.png)  

填写 clusterpolicy 配置参数  
![填写 clusterpolicy 配置](./images/14-cluster-policy-parameter.png)  

填写其中的 operator config 参数  
![填写 operator config 参数](./images/15-operator-config-parameter.png)  

填写其中的 DCMG Exporter config 参数  
![填写 DCMG Exporter config 参数](./images/16-DCMG-Exporter-config-parameter.png)  

填写其中的 device plugin config 参数  
![填写 device plugin config 参数](./images/17-device-plugin-config-parameter.png)  

填写其中的 driver config 参数  
![填写 driver config 参数](./images/18-driver-config-parameter.png)  

填写其中的 Group Feature Discovery Plugin config 参数  
![填写 Group Feature Discovery Plugin config 参数](./images/19-Group-Feature-Discovery-Plugin-config-parameter.png)  

填写其中的 container toolkit config 参数  
![填写 container toolkit config 参数](./images/20-container-toolkit-config-parameter.png)  

填写其中的 Dcmg 参数  
![填写 Dcmg 参数](./images/21-dcmg-parameter.png)  

填写其中的 Node Status Exporter 参数  
![填写 Node Status Exporter 参数](./images/22-Node-Status-Exporter-parameter.png)  

填写其中的 Mig Manager 参数  
![填写 Mig Manager 参数](./images/23-Mig-Manager-parameter.png)  

填写其中的 Validator 参数  
![填写 Validator 参数](./images/24-Validator-parameter.png)  

创建clusterpolicy成功  
![创建clusterpolicy成功](./images/25-cluster-policy-ready.png)  
